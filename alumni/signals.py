...
def log_alumni_change(sender, instance, created, **kwargs):
    """
    Create a notification when a Alumni is created or updated.
    """
    ...
    title = f"Alumni {action.capitalize()}"
    message = f"Alumni '{instance.name}' was {action} by {user.teacher.teacher_name}."
...
def log_alumni_deletion(sender, instance, **kwargs):
    """
    Create a notification when a Alumni is deleted.
    """
    user = get_current_user()
    if not user:
        return
    ...
    message = f"Alumni '{instance.name}' was deleted by {user.teacher.teacher_name}."
