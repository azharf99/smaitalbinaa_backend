...
def log_academic_calendar_change(sender, instance, created, **kwargs):
    """
    Create a notification when a Academic Calendar is created or updated.
    """
    ...
    title = f"Academic Calendar {action.capitalize()}"
    message = f"Academic Calendar '{instance.event_name}' was {action} by {user.teacher.teacher_name}."
...
@receiver(post_delete, sender=AcademicCalendar)
def log_academic_calendar_deletion(sender, instance, **kwargs):
    """
    Create a notification when a Academic Calendar is deleted.
    """
    user = get_current_user()
    if not user:
        return
    ...
    title = "Academic Calendar Deleted"
    # Use user.teacher.teacher_name for consistency and to avoid potential errors if user has no teacher profile.
    message = f"Academic Calendar '{instance.event_name}' was deleted by {user.teacher.teacher_name}."
