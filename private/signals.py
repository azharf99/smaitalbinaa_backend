...
def log_private_change(sender, instance, created, **kwargs):
    """
    Create a notification when a Private is created or updated.
    """
    ...
    title = f"Private {action.capitalize()}"
    message = f"Private {instance.tanggal_bimbingan} {instance.pelajaran.nama_pelajaran} was {action} by {user.teacher.teacher_name}."
...
@receiver(post_delete, sender=Private)
def log_private_deletion(sender, instance, **kwargs):
    """
    Create a notification when a Private is deleted.
    """
    user = get_current_user()
    if not user:
        return
    ...
    title = "Private Deleted"
    # Use user.teacher.teacher_name for consistency and to avoid potential errors if user has no teacher profile.
    message = f"Private {instance.tanggal_bimbingan} {instance.pelajaran.nama_pelajaran} was deleted by {user.teacher.teacher_name}."
