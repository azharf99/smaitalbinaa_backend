...
@receiver(post_save, sender=Extracurricular)
def log_extracurricular_change(sender, instance, created, **kwargs):
    """
    Create a notification when a Extracurricular is created or updated.
    """
    ...
    title = f"Extracurricular {action.capitalize()}"
    message = f"Extracurricular {instance.name} was {action} by {user.teacher.teacher_name}."
...
@receiver(post_delete, sender=Extracurricular)
def log_extracurricular_deletion(sender, instance, **kwargs):
    """
    Create a notification when a Extracurricular is deleted.
    """
    user = get_current_user()
    if not user:
        return
    ...
    message = f"Extracurricular {instance.name} was deleted by {user.teacher.teacher_name}."
