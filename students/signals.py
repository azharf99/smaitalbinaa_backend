...
def log_student_change(sender, instance, created, **kwargs):
    """
    Create a notification when a Student is created or updated.
    """
    ...
    message = f"Student {instance.student_name} was {action} by {user.teacher.teacher_name}."
...
def log_student_deletion(sender, instance, **kwargs):
    """
    Create a notification when a Student is deleted.
    """
    user = get_current_user()
    if not user:
        return
    ...
    message = f"Student {instance.student_name} was deleted by {user.teacher.teacher_name}."
